        ...
        <form action="{{ route('admin.store') }}" method="POST" class="space-y-4">
            ...
            <div class="flex flex-col md:flex-row md:space-x-4 space-y-4 md:space-y-0">
                <div class="flex-1 relative">
                    <label for="password" class="mb-1 block font-medium text-slate-700">Password</label>
                    <input type="password" name="password" id="password"
                        class="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 pr-10 text-slate-700 outline-none transition focus:border-slate-300 focus:bg-white focus:ring-2 focus:ring-slate-200" required>
                    <button type="button" id="togglePassword" class="absolute inset-y-0 mt-8 right-0 flex items-center pr-3 text-slate-500 hover:text-slate-700">
            ...
            <div class="mt-4">
                ...
                <div class="space-y-2 rounded-2xl border border-slate-200 bg-slate-50 p-4">
                    ...
                    <div class="flex items-center justify-center gap-20 text-slate-700 text-md ml-8 mr-8">
                        <span class="flex w-40 items-center gap-2">
                            <i class="{{ $module['icon'] }} w-4 text-slate-500"></i>
                            {{ $module['label'] }}
                        </span>
                        <label class="flex items-center gap-2"><input type="checkbox" name="permissions[{{ $key }}][]" value="create" class="rounded border-slate-300 text-slate-900 focus:ring-slate-300">Create</label>
                        <label class="flex items-center gap-2"><input type="checkbox" name="permissions[{{ $key }}][]" value="read" class="rounded border-slate-300 text-slate-900 focus:ring-slate-300">Read</label>
                        <label class="flex items-center gap-2"><input type="checkbox" name="permissions[{{ $key }}][]" value="update" class="rounded border-slate-300 text-slate-900 focus:ring-slate-300">Edit</label>
                        <label class="flex items-center gap-2"><input type="checkbox" name="permissions[{{ $key }}][]" value="delete" class="rounded border-slate-300 text-slate-900 focus:ring-slate-300">Delete</label>
                    </div>
                    @endforeach
            ...
            <div class="flex justify-center md:justify-start">
                <button type="submit"
                    class="w-full rounded-xl bg-slate-900 px-4 py-2 font-medium text-white transition hover:bg-slate-800 md:w-[150px]">
                    Create User
                </button>
            </div>
        </form>
