...
<div class="min-h-screen flex items-center justify-center bg-slate-50 px-4 py-10">
    <div class="w-full max-w-3xl rounded-2xl border border-slate-200 bg-white p-8 shadow-[0_24px_60px_rgba(15,23,42,0.12)]">
        ...
        <p class="mb-8 text-center text-sm text-slate-500">
            Update state details using the same light form style.
        </p>
        ...
        <form action="{{ route('states.update', $state->id) }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <textarea name="description"
                          rows="4"
                          class="w-full rounded-lg border border-slate-300 bg-white px-4 py-3 text-slate-900 outline-none transition focus:border-slate-900 focus:ring-2 focus:ring-slate-200">{{ old('description', $state->description) }}</textarea>
            </div>
